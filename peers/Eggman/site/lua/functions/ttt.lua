--- Tic-tac-toe game management — ORM DSL via goop.orm
--- @rate_limit 0
...
function lobby()
    ...
    local loss_rows = games:aggregate("COUNT(*) as n", {
        where = "winner != '' AND winner != ? AND status LIKE 'won_%'",
        args = { goop.self.id },
    })
    local draw_rows = games:aggregate("COUNT(*) as n", {
        where = "status = 'draw'",
    })
