...
| 5 | Publish endpoint abuse | **Fixed** — Per-IP sliding window rate limiting on `/publish` (60 req/min); returns HTTP 429 when exceeded |
...
| 17 | Script/state exposure | **Fixed** — Content store `cleanAbs()` blocks all `lua/` paths, matching P2P protocol behavior |
