...
| -- | -- | -- | -- |
| 1 | Direct chat | `directchat.New(selfID, store, mq)` | — |
| 2 | Lua engine | `luapkg.NewEngine(...)` (if enabled) | — |
| 3 | Group manager | `group.New(host, db, transport, resolvePeer)` | — |
...
| `/goop/data/1.0.0` | Remote ORM queries | Newline-delimited JSON (`DataRequest` → `DataResponse`) |
...
All route handlers receive `routes.Deps` — grouped by domain (see `viewer.md` for full field table):
